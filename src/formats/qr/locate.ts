// import { QRLocation } from '../models';
// import { BitMatrix } from './bitmatrix';
import { QRLocation } from './qrlocation';
import { BitMatrix } from '../../core/util/bitmatrix';
...
class FinderPattern {
    ...
    combineEstimate(i: number, j: number, newModuleSize: number): FinderPattern {
        let combinedCount = this.count + 1;
        let combinedX = (this.count * this.x + j) / combinedCount;
        let combinedY = (this.count * this.y + i) / combinedCount;
        let combinedModuleSize = (this.count * this.estimatedModuleSize + newModuleSize) / combinedCount;
...
function foundPatternCross(stateCount: number[]): boolean {
    ...
    let moduleSize = (totalModuleSize << INTEGER_MATH_SHIFT) / 7;
...
function centerFromEnd(stateCount: number[], end: number): number {
    let result = (end - stateCount[4] - stateCount[3]) - stateCount[2] / 2;
    // Fix this.
    if (result !== result) {
        return null;
    }
...
function distance(pattern1: FinderPattern, pattern2: FinderPattern): number {
    let a = pattern1.x - pattern2.x;
    let b = pattern1.y - pattern2.y;
...
function ReorderFinderPattern(patterns: FinderPattern[]): QRLocation {
    ...
    let pointA: FinderPattern, pointB: FinderPattern, pointC: FinderPattern;
...
export function locate(matrix: BitMatrix): QRLocation {
    // Global state :(
    let possibleCenters: FinderPattern[] = [];
    ...
    // Methods
    function crossCheckDiagonal(startI: number, centerJ: number, maxCount: number,
        originalStateCountTotal: number): boolean {
        let maxI = matrix.height;
        let maxJ = matrix.width;
        ...
        while ((startI - i >= 0) && (centerJ - i >= 0) && !get(centerJ - i, startI - i) && stateCount[1]
        <= maxCount) {
            stateCount[1]++;
            i++;
        }
        ...
        while ((startI + i < maxI) && (centerJ + i < maxJ) && !get(centerJ + i, startI + i) && stateCount[3]
        < maxCount) {
            stateCount[3]++;
            i++;
        }
        ...
        while ((startI + i < maxI) && (centerJ + i < maxJ) && get(centerJ + i, startI + i) && stateCount[4]
        < maxCount) {
            stateCount[4]++;
            i++;
        }
    ...
    function crossCheckVertical(startI: number, centerJ: number, maxCount: number,
    ...
    function haveMultiplyConfirmedCenters(): boolean {
        ...
        let totalModuleSize = 0;
        let max = possibleCenters.length;
        ...
        for (let i = 0; i < max; i++) {
            let pattern = possibleCenters[i];
            totalDeviation += Math.abs(pattern.estimatedModuleSize - average);
        }
        return totalDeviation <= 0.05 * totalModuleSize;
    }

    function crossCheckHorizontal(startJ: number, centerI: number, maxCount: number,
        originalStateCountTotal: number): number {
        let maxJ = matrix.width;
        let stateCount = [0, 0, 0, 0, 0];
    ...
    function handlePossibleCenter(stateCount: number[], i: number, j: number, pureBarcode: boolean): boolean {
        ...
        let centerI = crossCheckVertical(i, Math.floor(centerJ), stateCount[2], stateCountTotal);
        if (centerI != null) {
            // Re-cross check
            centerJ = crossCheckHorizontal(Math.floor(centerJ), Math.floor(centerI), stateCount[2], stateCountTotal);
            if (centerJ != null && (!pureBarcode || crossCheckDiagonal(Math.floor(centerI), Math.floor(centerJ),
                    stateCount[2], stateCountTotal))) {
                let estimatedModuleSize = stateCountTotal / 7;
                let found = false;
                for (let index = 0; index < possibleCenters.length; index++) {
                ...
                if (!found) {
                    // let point = new FinderPattern(centerJ.Value, centerI.Value, estimatedModuleSize);
        ...
        possibleCenters.forEach((center) => {
            if (center.count >= CENTER_QUORUM) {
                if (firstConfirmedCenter == null) {
                    ...
                    hasSkipped = true;
                    // UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing
                    // conversions.
                    return Math.floor(
                            Math.abs(firstConfirmedCenter.x - center.x) - Math.abs(firstConfirmedCenter.y - center.y))
                        / 2;
                    // return (int)(Math.Abs(firstConfirmedCenter.X - center.X) - Math.Abs(firstConfirmedCenter.Y -
                    // center.Y)) / 2;
                }
            }
        });
    ...
    function selectBestPatterns(): FinderPattern[] {
        let startSize = possibleCenters.length;
        if (startSize < 3) {
        ...
        if (startSize > 3) {
            ...
            let square = 0;
            possibleCenters.forEach((center) => {
                let size = center.estimatedModuleSize;
            ...
            possibleCenters.sort((x, y) => {
                let dA = Math.abs(y.estimatedModuleSize - average);
                let dB = Math.abs(x.estimatedModuleSize - average);
        ...
        if (possibleCenters.length > 3) {
            ...
            let average = totalModuleSize / possibleCenters.length;
    ...
    for (let i = iSkip - 1; i < maxI && !done; i += iSkip) {
                ...
                if ((currentState & 1) === 0) {
                    ...
                    if (currentState === 4) {
                        ...
                        if (foundPatternCross(stateCount)) {
                            // Yes
                            let confirmed = handlePossibleCenter(stateCount, i, j, pureBarcode);
                            if (confirmed) {
        ...
        if (foundPatternCross(stateCount)) {
            let confirmed = handlePossibleCenter(stateCount, i, maxJ, pureBarcode);
            if (confirmed) {
                iSkip = stateCount[0];
                if (hasSkipped) {
                    // Found a third one
