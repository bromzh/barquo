export class ReedSolomonDecoder {
    ...
    public decode(received: number[], twoS: number) {
        let poly = new GenericGFPoly(this.field, received);
        let syndromeCoefficients: number[] = new Array(twoS);
        let noError = true;
        for (let i = 0; i < twoS; i++) {
            let evaluation = poly.evaluateAt(this.field.exp(i + this.field.generatorBase));
        ...
        let syndrome = new GenericGFPoly(this.field, syndromeCoefficients);
        ...
        let errorMagnitudes = this.findErrorMagnitudes(omega, errorLocations);
    ...
    findErrorLocations(errorLocator: GenericGFPoly): number[] {
        ...
        let result: number[] = new Array(numErrors);
        let e = 0;
...
export class GenericGFPoly {
    ...
    addOrSubtract(other: GenericGFPoly): GenericGFPoly {
        ...
        let sumDiff: number[] = new Array(largerCoefficients.length);
    ...
    multiplyPoly(other: GenericGFPoly): GenericGFPoly {
        ...
        let product: number[] = new Array(aLength + bLength - 1);
    ...
    multiplyByMonomial(degree: number, coefficient: number): GenericGFPoly {
        ...
        let size = this.coefficients.length;
        let product: number[] = new Array(size + degree);
        for (let i = 0; i < size; i++) {
...
export class GenericGF {
    ...
    buildMonomial(degree: number, coefficient: number): GenericGFPoly {
        ...
        let coefficients: number[] = new Array(degree + 1);
