...
function aboutEquals(center: EstimatedPoint, moduleSize: number, i: number, j: number) {
    if (Math.abs(i - center.y) <= moduleSize && Math.abs(j - center.x) <= moduleSize) {
        let moduleSizeDiff = Math.abs(moduleSize - center.estimatedModuleSize);
...
function combineEstimate(center: EstimatedPoint, i: number, j: number, newModuleSize: number) {
    let combinedX = (center.x + j) / 2;
    let combinedY = (center.y + i) / 2;
    let combinedModuleSize = (center.estimatedModuleSize + newModuleSize) / 2;
...
function centerFromEnd(stateCount: number[], end: number) {
    let result = (end - stateCount[2]) - stateCount[1] / 2;
    if (isNaN(result)) {
        return null;
    }
    return result;
...
function crossCheckVertical(startI: number, centerJ: number, maxCount: number, originalStateCountTotal: number,
    moduleSize: number, image: BitMatrix) {
...
export function findAlignment(startX: number, startY: number, width: number, height: number, moduleSize: number,
    image: BitMatrix): Point {
    // Global State :(
    let possibleCenters: EstimatedPoint[] = [];
    ...
    function handlePossibleCenter(stateCount: number[], i: number, j: number, modSize: number): EstimatedPoint {
        let stateCountTotal = stateCount[0] + stateCount[1] + stateCount[2];
        let centerJ = centerFromEnd(stateCount, j);
        if (centerJ == null) {
            return null;
        }
        let centerI = crossCheckVertical(i, Math.floor(centerJ), 2 * stateCount[1], stateCountTotal, modSize, image);
        if (centerI != null) {
            let estimatedModuleSize = (stateCount[0] + stateCount[1] + stateCount[2]) / 3;
            //tslint:disable-next-line
            for (let i2 in possibleCenters) {
                let center = possibleCenters[i2];
            ...
            let point = { x: centerJ, y: centerI, estimatedModuleSize: estimatedModuleSize };
    ...
    for (let iGen = 0; iGen < height; iGen++) {
        ...
        if (foundPatternCross(stateCount, moduleSize)) {
            confirmed = handlePossibleCenter(stateCount, i, moduleSize, maxJ);
            if (confirmed != null) {
                return confirmed;
            }
        }
