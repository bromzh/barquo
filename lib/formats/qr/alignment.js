...
export function findAlignment(startX, startY, width, height, moduleSize, image) {
    // Global State :(
    var possibleCenters = [];
    ...
    function handlePossibleCenter(stateCount, i, j, modSize) {
        var stateCountTotal = stateCount[0] + stateCount[1] + stateCount[2];
        var centerJ = centerFromEnd(stateCount, j);
        if (centerJ == null) {
            return null;
        }
        var centerI = crossCheckVertical(i, Math.floor(centerJ), 2 * stateCount[1], stateCountTotal, modSize, image);
        if (centerI != null) {
