        ...
        possibleCenters.forEach(function (center) {
            if (center.count >= CENTER_QUORUM) {
                if (firstConfirmedCenter == null) {
                    ...
                    // conversions.
                    return Math.floor(Math.abs(firstConfirmedCenter.x - center.x) - Math.abs(firstConfirmedCenter.y - center.y))
                        / 2;
                }
            }
        });
    ...
    function selectBestPatterns() {
        ...
        if (startSize > 3) {
            ...
            possibleCenters.sort(function (x, y) {
                var dA = Math.abs(y.estimatedModuleSize - average_1);
                var dB = Math.abs(x.estimatedModuleSize - average_1);
        ...
        if (possibleCenters.length > 3) {
            ...
            var totalModuleSize_2 = 0;
            possibleCenters.forEach(function (possibleCenter) {
                totalModuleSize_2 += possibleCenter.estimatedModuleSize;
            });
            var average_2 = totalModuleSize_2 / possibleCenters.length;
            // possibleCenters.Sort(new CenterComparator(average));
            possibleCenters.sort(function (x, y) {
                if (y.count === x.count) {
                    var dA = Math.abs(y.estimatedModuleSize - average_2);
                    var dB = Math.abs(x.estimatedModuleSize - average_2);
                    return dA < dB ? 1 : dA === dB ? 0 : -1;
                }
